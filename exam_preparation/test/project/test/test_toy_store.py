...
class TestToyStore(TestCase):
    ...
    def test_remove_toy_from_num_existing_shelf_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            self.store.remove_toy("Z", "HotWheels")

        self.assertEqual("Shelf doesn't exist!", str(ex.exception))

    def test_remove_none_existing_toy_from_shelf_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            self.store.remove_toy("A", "HotWheels")

        self.assertEqual("Toy in that shelf doesn't exist!", str(ex.exception))
